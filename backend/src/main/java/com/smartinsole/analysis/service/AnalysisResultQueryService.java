...
import com.smartinsole.analysis.dto.AnalysisDtos.AnalysisPendingResponse;
import com.smartinsole.analysis.dto.AnalysisDtos.AnalysisResultResponse;
import com.smartinsole.analysis.dto.AnalysisDtos.DataQualityResult;
import com.smartinsole.analysis.dto.AnalysisDtos.GaitSummary;
...
import com.smartinsole.analysis.dto.AnalysisDtos.PatternResult;
import com.smartinsole.analysis.dto.AnalysisDtos.PressureDistribution;
import com.smartinsole.analysis.dto.AnalysisDtos.RecommendationSummary;
...
public class AnalysisResultQueryService {
    ...
    public ResponseEntity<Object> result(UUID sessionId, UUID userId) {
        ...
        // Likewise the rule-v1.3.0 stride columns and the load share inside the distribution JSON stay null
        // for older results, and the rule-v1.4.0 movement summary (V9 JSON) for results before it or
        // sessions without IMU frames.
        ...
        MovementSummary movementSummary = result.getMovementSummaryJson() == null ? null
                : parse(result.getMovementSummaryJson(), new TypeReference<>() { });
        ...
        return ResponseEntity.ok(new AnalysisResultResponse(sessionId, "COMPLETED", result.getAlgorithmVersion(),
                ...
                new GaitSummary(result.getCadence(), result.getLeftContactTimeMs(),
                        ...
                        result.getLeftStrideTimeMs(), result.getRightStrideTimeMs(), result.getMeanStrideTimeMs()),
                parse(result.getPressureDistributionJson(), new TypeReference<>() { }), patternResponses,
                observationSummary, movementSummary, recommendationResponses, DISCLAIMER, result.getCreatedAt()));
    }
