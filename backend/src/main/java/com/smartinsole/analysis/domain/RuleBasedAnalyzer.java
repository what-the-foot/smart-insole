...
import com.smartinsole.analysis.dto.AnalysisDtos.CenterOfPressure;
...
/**
 * rule-v1.3.0: quality -> calibration -> smoothing -> contact windows (valid steps) -> per-window
 ...
 * rule-v1.3.0 adds two session-level descriptors on top of the unchanged rule-v1.2.0 computation: the
 * left/right load share (each foot's mean contact-frame total as a share of both feet, a relative signal
 * share and not force or weight) and the per-foot stride time (median start-to-start interval of
 * consecutive contact windows in device time). rule-v1.4.0 adds the IMU shank movement summary
 * ({@link ImuMovementAnalyzer}) computed from the stored accel/gyro samples over the same contact windows;
 * the pressure computation is unchanged.
 */
@Component
public class RuleBasedAnalyzer {
    ...
    public ComputedAnalysis analyze(MeasurementSession session, List<StoredPressureFrame> rawFrames,
        ...
        // Sorted once per foot by the receiver-unwrapped sequence, which stays monotone across a device-time
        // RESET (deviceTimeMs going backwards); the per-side raw lists below keep exactly the index order of the
        // smoothed per-side frames, so the contact windows index both the pressure frames and the IMU samples.
        List<StoredPressureFrame> sortedRaw = rawFrames.stream()
                .sorted(Comparator.comparing(StoredPressureFrame::footSide)
                        .thenComparingLong(StoredPressureFrame::sequence))
                .toList();
        List<NormalizedFrame> calibrated = sortedRaw.stream()
        ...
        // Load share compares raw frame totals, so it is only meaningful when both feet have the same sensor count.
        int leftSensors = contexts.get(FootSide.LEFT).points().size();
        int rightSensors = contexts.get(FootSide.RIGHT).points().size();
        LoadShare loadShare = leftSensors == rightSensors
                ? loadShare(meanContactTotal(leftFrames, leftWindows), meanContactTotal(rightFrames, rightWindows))
                : LoadShare.NONE;
        PressureDistribution responseDistribution = pressureDistribution(leftDistribution, rightDistribution,
                sensorShare(leftFrames, leftWindows), sensorShare(rightFrames, rightWindows), loadShare);
        Double leftStride = strideTimeMs(leftWindows);
        Double rightStride = strideTimeMs(rightWindows);
        MovementSummary movementSummary = ImuMovementAnalyzer.analyze(
                sortedRaw.stream().filter(frame -> frame.footSide() == FootSide.LEFT).toList(), leftWindows,
                sortedRaw.stream().filter(frame -> frame.footSide() == FootSide.RIGHT).toList(), rightWindows,
                session.getSampleRateHz(), properties.movement());
        ...
        return new ComputedAnalysis(quality.score(), quality.level(), quality.missingRate(),
                ...
                rightContact.averageDurationMs(), symmetry, validStepCount, leftStride, rightStride,
                meanStrideTimeMs(leftStride, rightStride), responseDistribution,
    ...
    private static List<NormalizedFrame> movingAverage(List<NormalizedFrame> input) {
        ...
        for (FootSide side : FootSide.values()) {
            List<NormalizedFrame> frames = bySide.getOrDefault(side, List.of()).stream()
                    .sorted(Comparator.comparingLong(NormalizedFrame::sequence)).toList();
        ...
        return result.stream().sorted(Comparator.comparing(NormalizedFrame::footSide)
    ...
    /**
     * Splits one foot's frames (already in sequence order) into contact windows: runs of frames whose total
     * is at or above the contact threshold. A device-time gap longer than three sample periods, or device
     * time going backwards (a receiver RESET), closes the current window so a window never spans a
     * transmission hole or a device restart.
     */
    static List<ContactWindow> contactWindows(List<Long> deviceTimes, List<Double> pressureTotals,
        ...
        for (int index = 0; index < deviceTimes.size(); index++) {
            ...
            if (previousTime != null && start != null
                    && (deviceTime - previousTime > maximumContiguousDelta || deviceTime < previousTime)) {
    ...
    static PressureDistribution pressureDistributionFromValues(
        ...
        return pressureDistribution(distributionValues(leftFrames, leftPoints),
                distributionValues(rightFrames, rightPoints), null, null, LoadShare.NONE);
    }
    ...
    private static PressureDistribution pressureDistribution(DistributionMetrics left, DistributionMetrics right,
        ...
        return new PressureDistribution(left.medialRatio(), left.lateralRatio(),
                ...
                leftShare, rightShare, loadShare.leftPct(), loadShare.rightPct());
    }
    ...
    /**
     * Mean frame total (sum of the normalised 0..100 sensor values) over the frames inside the foot's
     * contact windows. Null when the foot has no contact window.
     */
    static Double meanContactTotal(List<NormalizedFrame> frames, List<ContactWindow> windows) {
        double sum = 0;
        int counted = 0;
        for (ContactWindow window : windows) {
            for (int index = window.startIndex(); index <= window.endIndex(); index++) {
                sum += total(frames.get(index).values());
                counted++;
            }
        }
    ...
    /**
     * rule-v1.3.0 left/right load share: L/(L+R) x 100 and R/(L+R) x 100 of the two mean contact-frame
     * totals. A relative signal share, not force or weight. Both values are null when either foot has no
     * contact window (or the totals sum to zero).
     */
    static LoadShare loadShare(Double leftMeanTotal, Double rightMeanTotal) {
        if (leftMeanTotal == null || rightMeanTotal == null) return LoadShare.NONE;
        double sum = leftMeanTotal + rightMeanTotal;
        if (sum <= 0) return LoadShare.NONE;
        return new LoadShare(leftMeanTotal / sum * 100.0, rightMeanTotal / sum * 100.0);
    }

    /**
     * rule-v1.3.0 stride time estimate of one foot: the median start-to-start interval (deviceTimeMs)
     * between consecutive contact windows, in the device-time order produced by contactWindows. Null with
     * fewer than two windows. A contact-window based estimate, not a clinically validated gait cycle.
     */
    static Double strideTimeMs(List<ContactWindow> windows) {
        if (windows.size() < 2) return null;
        double[] intervals = new double[windows.size() - 1];
        for (int index = 1; index < windows.size(); index++) {
            intervals[index - 1] = windows.get(index).startDeviceTimeMs() - windows.get(index - 1).startDeviceTimeMs();
        }
        java.util.Arrays.sort(intervals);
        int middle = intervals.length / 2;
        return intervals.length % 2 == 1 ? intervals[middle] : (intervals[middle - 1] + intervals[middle]) / 2.0;
    }

    /** (left + right) / 2 when both feet have a stride time, the single available value otherwise, else null. */
    static Double meanStrideTimeMs(Double left, Double right) {
    ...
    record ContactWindow(int startIndex, int endIndex, long startDeviceTimeMs, long endDeviceTimeMs,
                         double durationMs) { }
    /** rule-v1.3.0 left/right load share in percent; both null when unavailable. */
    record LoadShare(Double leftPct, Double rightPct) {
        static final LoadShare NONE = new LoadShare(null, null);
    }
