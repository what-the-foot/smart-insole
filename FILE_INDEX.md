...
- `contracts/openapi.yaml`: REST API 단일 계약
- `contracts/realtime-message.schema.json`: STOMP payload 계약
- `fixtures/manifest.json`: fixture 목적과 파일 매핑
- `fixtures/frame-batch-*.json`: 정상, 6센서, 중복, gap, 역순, stuck, 비대칭, 한쪽 단절 입력
...
- `backend/src/main/resources/db/migration/`: schema, seed, 품질·조회 index migration
...
- `backend/src/main/java/com/smartinsole/measurement/`: 세션 상태, 기록, JDBC batch 수신, 품질 통계
- `backend/src/main/java/com/smartinsole/realtime/`: snapshot, CoP, 10Hz STOMP, 연결·구독 권한
- `backend/src/main/java/com/smartinsole/analysis/`: durable job, 재시도·재시작 복구, 규칙 분석, 결과
- `backend/src/main/java/com/smartinsole/recommendation/`: 추천 상세 조회와 결과 연결
- `backend/src/test/`: 도메인·통합·보안·동시성·MySQL Testcontainers 검증
...
- `scripts/verify-all.{sh,ps1}`: 계약 → backend → frontend → 선택 E2E
- `scripts/mock-receiver/`: 결정적 100Hz 합성 Receiver
