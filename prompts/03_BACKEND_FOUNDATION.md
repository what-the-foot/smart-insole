...
### BE-001 — Spring Boot 기반

- 기존 backend가 없을 때만 Java 21 + Gradle Spring Boot 프로젝트를 생성합니다.
- Web, Validation, JPA, JDBC, Security, WebSocket, Actuator, MySQL, Flyway와 테스트 기반을 설정합니다.
- Gradle Wrapper를 포함하고 버전을 재현 가능하게 고정합니다.
- local/test 설정을 분리하고 운영 secret은 환경변수 placeholder만 둡니다.
- UTC 기준 `Clock` 또는 공통 시간 정책의 기반을 만듭니다.
- 빈 MySQL에서 Flyway가 적용되고 `/actuator/health`를 확인할 수 있어야 합니다.
- 패키지는 기존 프로젝트 기본 패키지가 있으면 그것을 유지합니다.

### BE-002 — 오류·Validation·Clock·Actuator

- `ErrorCode`, `BusinessException`, `ErrorResponse`, `GlobalExceptionHandler`를 구현합니다.
- Bean Validation, 잘못된 JSON, 비즈니스 오류, 인증 401, 권한 403, 예상 밖 500 응답 형식을 통일합니다.
- 응답에 추적 가능한 trace ID와 UTC timestamp를 포함하되 내부 SQL·stack trace는 노출하지 않습니다.
- 테스트에서 제어 가능한 `Clock` 사용 방식을 확정합니다.
- Actuator 공개 범위를 최소화합니다.
...
- 예상 밖 오류의 안전한 500 응답
- UTC timestamp
- 빈 MySQL migration(Testcontainers 권장)
...
H2에서만 통과한 핵심 MySQL 동작을 완료로 간주하지 마세요.
