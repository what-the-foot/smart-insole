    ...
    <div className="app-frame">
      ...
      <header className="topbar">
        ...
        <nav aria-label="주요 메뉴" className={menuOpen ? 'main-nav main-nav--open' : 'main-nav'}>
          {navItems.map((item) => (
            <NavLink key={item.to} className={({ isActive }) => `nav-link${isActive ? ' nav-link--active' : ''}`} onClick={() => setMenuOpen(false)} to={item.to}>
              <Icon name={item.icon} /><span>{item.label}</span>
            </NavLink>
          ))}
          <button className="nav-link mobile-signout" onClick={handleSignout} type="button">
            <Icon name="logout" /><span>로그아웃</span>
          </button>
        </nav>
        <div className="user-menu">
          <span className="user-chip"><Icon name="user" /><span><small>안녕하세요</small>{user?.name}</span></span>
          <button aria-label="로그아웃" className="icon-button icon-button--quiet" onClick={handleSignout} title="로그아웃"><Icon name="logout" /></button>
