...
import type { AnalysisResultResponse, MeasurementSessionResponse, ObservationLevel, ObservationSummaryItem, PatternResult, SensorSharePct } from '../../api/types';
...
const observationTone = (level: ObservationLevel) =>
  level === 'REPEATEDLY_OBSERVED' ? 'warning' as const : level === 'PARTIALLY_OBSERVED' ? 'info' as const : 'neutral' as const;
    ...
    <div className="result-stack">
      ...
      <section className="content-card" aria-labelledby="distribution-title">
        ...
        <div className="distribution-grid">
          <DistributionFoot label="왼발" medial={result.pressureDistribution.leftMedialRatio} lateral={result.pressureDistribution.leftLateralRatio} heel={result.pressureDistribution.leftHeelRatio} midfoot={result.pressureDistribution.leftMidfootRatio} forefoot={result.pressureDistribution.leftForefootRatio} peak={result.pressureDistribution.leftPeakPressure} meanCoP={result.pressureDistribution.leftMeanCoP} sharePct={result.pressureDistribution.leftSensorSharePct ?? null} side="left" />
          <DistributionFoot label="오른발" medial={result.pressureDistribution.rightMedialRatio} lateral={result.pressureDistribution.rightLateralRatio} heel={result.pressureDistribution.rightHeelRatio} midfoot={result.pressureDistribution.rightMidfootRatio} forefoot={result.pressureDistribution.rightForefootRatio} peak={result.pressureDistribution.rightPeakPressure} meanCoP={result.pressureDistribution.rightMeanCoP} sharePct={result.pressureDistribution.rightSensorSharePct ?? null} side="right" />
        </div>
        <p className="metric-note">전족부는 발가락 영역을 포함합니다. {resultTerms.peakSignal}와 평균 {resultTerms.estimatedCop}은 세션 ADC 기준 0~100 상대 신호와 0~1 좌표계의 분석 요약이며, 보정된 압력값이나 의료 진단값이 아닙니다. {resultTerms.signalShare}은 접촉 프레임 평균에서 센서/전체합×100으로 계산한 값입니다.</p>
      </section>
      ...
      <section aria-labelledby="pattern-title">
        ...
        {result.patterns.length ? <div className="pattern-list">{result.patterns.map((pattern) => <PatternCard key={`${pattern.code}-${pattern.title}`} pattern={pattern} />)}</div> : <div className="no-pattern"><Icon name="check" /><div><h3>이번 측정에서 표시할 주요 패턴이 없습니다.</h3><p>반복 관찰된 경향이 없었다는 뜻일 뿐이며, 질환 유무나 건강 상태를 확정하는 의미가 아닙니다.</p></div></div>}
      </section>

      <section className="content-card" aria-labelledby="observation-title">
        <div className="section-heading"><div><p className="eyebrow">OBSERVATION SUMMARY</p><h2 id="observation-title">관찰 단계 요약</h2></div></div>
          ...
          <div className="observation-summary">
            {groupObservationSummary(result.observationSummary).map((group) => (
              <div className="observation-group" key={group.level}>
                <div className="observation-group__title"><StatusBadge tone={observationTone(group.level)}>{observationLevelLabels[group.level]}</StatusBadge><small>{group.items.length}종</small></div>
                {group.items.length ? <ul>{group.items.map((item) => <li key={item.code}><strong>{patternCodeLabel(item.code)}</strong><span>{occurrenceRateText(item)}</span></li>)}</ul> : <p className="observation-group__empty">해당 단계의 패턴이 없습니다.</p>}
              </div>
            ))}
            <p className="metric-note">유효 걸음(접촉 구간)을 창으로 삼아 각 패턴이 관찰된 비율입니다. 단계 경계는 기능 검증용 제안값이며 질환 판단이 아닙니다.</p>
          </div>
        ) : <p className="observation-summary__legacy"><Icon name="alert" />{OBSERVATION_LEVEL_UNAVAILABLE} — 분석 버전 {result.algorithmVersion} 결과에는 관찰 단계 정보가 없습니다.</p>}
      </section>
...
function PatternCard({ pattern }: { pattern: PatternResult }) {
  ...
  return (
    <article className="pattern-card">
      <div className="pattern-card__top"><span className="pattern-card__icon"><Icon name="activity" /></span><div><div className="pattern-card__badges"><StatusBadge tone={severityTone(pattern.severity)}>{severityLabel[pattern.severity]}</StatusBadge>{level ? <StatusBadge tone={observationTone(level)}>{observationLevelLabels[level]}</StatusBadge> : null}</div><h3>{pattern.title}</h3></div></div>
      <p>{pattern.message}</p>
      <p className="pattern-card__occurrence">{occurrence ?? OBSERVATION_LEVEL_UNAVAILABLE}</p>
      <div className="evidence-box"><strong>관찰 근거</strong><p>{pattern.evidence}</p></div>
    </article>
  );
}

function DistributionFoot({ label, medial, lateral, heel, midfoot, forefoot, peak, meanCoP, sharePct, side }: { label: string; medial: number; lateral: number; heel: number; midfoot: number | null; forefoot: number | null; peak: number | null; meanCoP: { x: number; y: number } | null; sharePct: SensorSharePct | null; side: 'left' | 'right' }) {
  return <article className="distribution-foot"><div className="distribution-foot__title"><span className={`device-side device-side--${side}`}>{side === 'left' ? 'L' : 'R'}</span><h3>{label}</h3></div><div className="ratio-group"><strong>좌우 방향</strong><RatioBar label="내측" ratio={medial} /><RatioBar label="외측" ratio={lateral} /></div><div className="ratio-group"><strong>발 길이 방향</strong><RatioBar label="뒤꿈치" ratio={heel} /><RatioBar label="중족부" ratio={midfoot} /><RatioBar label="전족부·발가락" ratio={forefoot} /></div>{sharePct ? <div className="ratio-group share-bars"><strong>{resultTerms.signalShare} (레이아웃 index 순)</strong>{sharePct.map((share, index) => <div className="ratio-row share-bar" key={index}><div><span>#{index + 1}</span><strong>{formatNumber(share)}%</strong></div><div className="ratio-track" aria-hidden="true"><span style={{ width: `${Math.max(0, Math.min(100, share))}%` }} /></div></div>)}</div> : null}<div className="distribution-summary"><div><span>{resultTerms.peakSignal}</span><strong>{peak === null ? '제공 안 됨' : `${formatNumber(peak)} / 100`}</strong></div><div><span>평균 {resultTerms.estimatedCop}</span><strong>{meanCoP ? `x ${formatNumber(meanCoP.x, 2)} · y ${formatNumber(meanCoP.y, 2)}` : '데이터 없음'}</strong></div></div></article>;
}
