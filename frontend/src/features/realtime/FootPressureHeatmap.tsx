...
export function FootPressureHeatmap({
  ...
  return (
    <figure
      className={`heatmap-card${disconnected ? ' heatmap-card--disconnected' : ''}`}
      aria-labelledby={`${side}-heatmap-title`}
    >
      <div className="heatmap-card__header">
        <div>
          ...
          <div>
            ...
            <h2 id={`${side}-heatmap-title`}>{label} 센서 신호</h2>
          </div>
        </div>
      ...
      <div className="heatmap-visual">
        <svg aria-describedby={`${side}-heatmap-summary`} role="img" viewBox="0 0 220 400">
          ...
          <path
            className="foot-outline"
            d="M109 371c-36 0-58-30-57-69 1-30 17-55 24-77 8-24 3-48 1-74-2-37 6-81 30-108 17-19 48-24 62-5 13 18 6 49-3 68-9 20-13 38-7 63 7 30 20 62 18 99-3 59-26 103-68 103Z"
            transform={side === 'LEFT' ? 'translate(229 0) scale(-1 1)' : undefined}
          />
          ...
          {data?.cop ? (
            <g
              ...
              transform={`translate(${heatmapX(data.cop.x, side)} ${42 + data.cop.y * 310})`}
            >
              <circle r="8" />
...
export function PressureLegend() {
  return (
    <div className="pressure-legend" aria-label="상대 신호 범례: 0은 낮음, 100은 높음">
      <span>낮음</span>
      <div aria-hidden="true" />
      <span>높음</span>
      <strong>0–100 상대값 · 원 아래 %는 {resultTerms.signalShare}</strong>
    </div>
  );
}
