...
import { contactStateLabels } from '../../utils/labels';
...
export function FootPressureHeatmap({
  ...
  const sensorSummary = data
    ? `${data.sensorValues.length}개 센서, 상대 총압력 ${formatNumber(data.totalPressure)}, ${contactStateLabels[data.contactState]}`
    : '아직 센서 데이터가 없습니다.';
  ...
  return (
    <figure className={`heatmap-card${disconnected ? ' heatmap-card--disconnected' : ''}`} aria-labelledby={`${side}-heatmap-title`}>
      <div className="heatmap-card__header">
        <div><span className={`device-side device-side--${side.toLowerCase()}`}>{side === 'LEFT' ? 'L' : 'R'}</span><div><p className="eyebrow">{side}</p><h2 id={`${side}-heatmap-title`}>{label} 압력</h2></div></div>
      ...
      <figcaption id={`${side}-heatmap-summary`} className="heatmap-summary">
        <div><span>상대 총압력</span><strong>{data ? formatNumber(data.totalPressure) : '—'}</strong></div>
        <div><span>접촉 상태</span><strong>{data ? contactStateLabels[data.contactState] : '대기'}</strong></div>
        <div><span>압력 중심</span><strong>{data?.cop ? `${formatNumber(data.cop.x, 2)}, ${formatNumber(data.cop.y, 2)}` : '감지 안 됨'}</strong></div>
...
export function PressureLegend() {
  return <div className="pressure-legend" aria-label="상대 압력 범례: 0은 낮음, 100은 높음"><span>낮음</span><div aria-hidden="true" /><span>높음</span><strong>0–100 상대값</strong></div>;
}
