      ...
      {showForm ? (
        <section className="content-card registration-card" aria-labelledby="register-device-title">
          ...
          <form className="device-form" onSubmit={handleSubmit}>
            ...
            <label className="field"><span>센서 수</span><select aria-describedby="sensor-count-help" onChange={(event) => updateSensorCount(Number(event.target.value) as SupportedSensorCount)} value={form.sensorCount}><option value={8}>8개 (S01..S08)</option><option disabled={!isSensorCountRegistrable(6)} value={6}>6개 (등록 불가)</option></select><small id="sensor-count-help">{SIX_SENSOR_REGISTRATION_NOTE}</small></label>
            <label className="field"><span>센서 배치 버전</span><input aria-describedby="layout-version-help" readOnly value={form.sensorLayoutVersion} /><small id="layout-version-help">센서 수에 맞는 활성 기본 배치를 자동으로 선택합니다. 레거시 배치(layout-v1, layout-v1-6)는 조회만 가능하고 새로 등록할 수 없습니다.</small></label>
            <label className="field"><span>ADC 최댓값</span><input aria-describedby="adc-max-help" readOnly value={form.adcMax} /><small id="adc-max-help">RAW ADC 12비트 스케일입니다. 현재 4095만 등록할 수 있습니다.</small></label>
